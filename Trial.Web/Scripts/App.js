(function ($) {
    ...
    p._ajax = function (url, data, type, isFile, callback) {
        if (isFile) {
            $.ajax({
                type: type,
                url: url,
                ...
                error: function (xhr) {
                    p._sendMessage(xhr.statusText, "error");
                    callback(false);
                }
            });
        } else {
    ...
    // messages
    p._sendMessage = function (title, statu) {
        ...
        toastr[statu.toString().toLowerCase()]("", title);
    };
